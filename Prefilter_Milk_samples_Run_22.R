...
# Function to parse taxonomy strings
...
Mother_Milk <- import_biom("./table-dada2_QR22G.biom", parseFunction = parse_taxonomy_greengenes)
## Metadata
New_sample_data <- read.table(file="./Run_22_Mapping_file_Run22_Good.csv", fill=TRUE, header=TRUE, sep="\t", row.names=1)
...
New_sample_data2 <- read.table(file="./Run_22_Mapping_file_Run22_Merged_Good.csv", fill=TRUE, header=TRUE, sep="\t", row.names=1)
...
#Parse taxonomy
Tax <- data.frame(read.table(file="./Taxonomy_QR22.tsv", header=TRUE, sep="\t", row.names=1))
as.character(Tax$Taxon)
taxonomy_table <- parse_phyloseq_taxonomy(Tax)
taxonomy_table

Mother_Milk_C22 <- phyloseq(otu_table(Mother_Milk_FM),  tax_table(taxonomy_table), sample_data(Mother_Milk_FM))
...
save(Mother_Milk_C22, file="./Run_22_Milk_Features_Phyloseq_Obj_Unfiltered.RData")
