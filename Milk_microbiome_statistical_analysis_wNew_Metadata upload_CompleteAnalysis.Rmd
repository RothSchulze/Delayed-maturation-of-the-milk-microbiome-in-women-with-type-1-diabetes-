...
##Obtain Ids for infant gut analysis from the other markdown
...
X <- X[!X %in% c("NSW-SYD-STG-005/006", " VIC-MEL-RMH-034")]
...
OT1DsB1 <- ROPH2$contrasts$p.value[1]
OT1DsB2 <- ROPH2$contrasts$p.value[2]
#Differences in observed richness due to T1D status are significant only in Time_Corrpoints B1 and B2.

##Analysis (reviewers 2 and 3) — GRS within non-T1D mothers only

#Richness

nonT1D <- subset(DivCal_Sort, t1dfactor == "nonT1D" & !is.na(biological_mother_GRS1))

richness_modelGRS <- glmmTMB(Observed ~ Time_Corr*biological_mother_GRS1 + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D, family = nbinom2())
summary(richness_modelGRS)
Anova(richness_modelGRS, type = "III") 

#Create a z-scored GRS variable
nonT1D$GRS_z <- scale(nonT1D$biological_mother_GRS1)
richness_modelGRSz <- glmmTMB(Observed ~ Time_Corr*GRS_z + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D, family = nbinom2())
summary(richness_modelGRSz)
Anova(richness_modelGRSz, type = "III") 

#At B2

nonT1D_B2 <- subset(nonT1D, Time_Corr == "B2")
richness_modelGRSB2 <- glmmTMB(Observed ~ biological_mother_GRS1 + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D_B2, family = nbinom2())
summary(richness_modelGRSB2)

#Create a z-scored GRS variable for B2
nonT1D_B2$GRS_z <- scale(nonT1D_B2$biological_mother_GRS1) #center GRS at 0 and scale it to 1 SD
nonT1D_B2$GRS_z <- as.numeric(nonT1D_B2$GRS_z)
richness_modelGRSB2z <- glmmTMB(Observed ~ GRS_z + DeliveryMode +  infant_sex + ParityBinary  + (1 | Run), data = nonT1D_B2, family = nbinom2())
summary(richness_modelGRSB2z)

#InvSimpson
IS_modelGRS <- glmmTMB(InvSimpson ~ Time_Corr*biological_mother_GRS1 + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D, family = Gamma(link = "log"))
summary(IS_modelGRS)
Anova(IS_modelGRS, type = "III") 

#Create a z-scored GRS variable
IS_modelGRSz <- glmmTMB(InvSimpson ~ Time_Corr*GRS_z + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D, family = Gamma(link = "log"))
summary(IS_modelGRSz)
Anova(IS_modelGRSz, type = "III") 

#At B2
IS_modelGRSB2 <- glmmTMB(InvSimpson ~ biological_mother_GRS1 + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = nonT1D_B2, family = Gamma(link = "log"))
summary(IS_modelGRSB2)

#Create a z-scored GRS variable for B2
nonT1D_B2$GRS_z <- scale(nonT1D_B2$biological_mother_GRS1) #center GRS at 0 and scale it to 1 SD
nonT1D_B2$GRS_z <- as.numeric(nonT1D_B2$GRS_z)
richness_modelGRSB2z <- glmmTMB(InvSimpson ~ GRS_z + DeliveryMode +  infant_sex + ParityBinary  + (1 | Run), data = nonT1D_B2, family = Gamma(link = "log"))
summary(richness_modelGRSB2z)

##Analysis (reviewers 2 and 3) — HbA1c within T1D mothers only

HbA1c_Meta <- read.table(file="/Users/alexandrajrothschulze/Documents/ENDIA Latest work/Milk Project 2025/Milk manuscript September 2025/Diabetologia submission/Revised/Submit to Diabetologia/Analysis for reviewers/Adjusting for HbA1c/HbA1c.txt", fill=TRUE, header=TRUE, sep="\t", row.names=1)

T1D_only <- subset(DivCal_Sort, t1dfactor == "T1D")

T1D_only <- T1D_only[order(rownames(T1D_only)), ]
T1D_onlyM <- cbind(T1D_only, HbA1c_Meta)
#mmol
T1D_onlyM$HbA1c_z <- scale(T1D_onlyM$hba1c_concentration_t03)

#Richness 

richness_model_HbA1c <- glmmTMB(
  Observed ~ Time_Corr * HbA1c_z +
    DeliveryMode + infant_sex + ParityBinary +
    (1 | biological_mother_id) + (1 | Run),
  data = T1D_onlyM,
  family = nbinom2()
)

summary(richness_model_HbA1c)
Anova(richness_model_HbA1c, type = "III")

#At B2
T1D_B2 <- subset(T1D_onlyM, Time_Corr == "B2")

richness_model_HbA1c_B2 <- glmmTMB(
  Observed ~ HbA1c_z +
    DeliveryMode + infant_sex + ParityBinary +
    (1 | Run),
  data = T1D_B2,
...
summary(richness_model_HbA1c_B2)

#InvSimpson

IS_model_HbA1c <- glmmTMB(
  InvSimpson ~ Time_Corr * HbA1c_z +
    DeliveryMode + infant_sex + ParityBinary +
    (1 | biological_mother_id) + (1 | Run),
  data = T1D_onlyM,
...
summary(IS_model_HbA1c)
Anova(IS_model_HbA1c, type = "III")

#At B2
T1D_B2 <- subset(T1D_onlyM, Time_Corr == "B2")

IS_model_HbA1c_B2 <- glmmTMB(
  InvSimpson ~ HbA1c_z +
    DeliveryMode + infant_sex + ParityBinary +
    (1 | Run),
  data = T1D_B2,
...
summary(IS_model_HbA1c_B2)


#What happens if the model is adjusted for "maternal_HLA_6DR_biological"
DivCal_Sort_HLA <- subset(DivCal_Sort, !is.na(maternal_HLA_6DR_biological))
DivCal_Sort_HLA$maternal_HLA_6DR_biological <- as.factor(DivCal_Sort_HLA$maternal_HLA_6DR_biological)
DivCal_Sort$HLA_cat <- factor(DivCal_Sort$maternal_HLA_6DR_biological, levels = c("DRXX", "DR33", "DR3X", "DR44", "DR4X", "DR34"))

#Richness
richness_model_HLA <- glmmTMB(Observed ~ Time_Corr*t1dfactor + HLA_cat + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = DivCal_Sort, family = nbinom2())
summary(richness_model_HLA)
Anova(richness_model_HLA, type = "III")

#InvSimpson
IS_model_HLA <- glmmTMB(InvSimpson ~ Time_Corr*t1dfactor + HLA_cat + DeliveryMode +  infant_sex + ParityBinary + (1 | biological_mother_id) + (1 | Run), data = DivCal_Sort, family = Gamma(link = "log"))
summary(IS_model_HLA)
Anova(IS_model_HLA, type = "III")


#ASSOCIATIONS WITH IgA measurements in B2 and V1

DivCal_Sort_S <- subset(DivCal_Sort, Time_Corr%in%c("B2", "V1"))
IgA_M <- read.table(file="./IgA_Measurements_B2&V1.txt", fill=TRUE, header=TRUE, sep="\t", row.names=1)
IGA <- rownames(IgA_M)
DivCal_Sort_S_Names <- rownames(DivCal_Sort_S)
shared <- intersect(IGA, DivCal_Sort_S_Names)

DivCal_Sort_IgA <- subset(DivCal_Sort_S, Merging%in%c(shared))
IgA_M$Name <- rownames(IgA_M)
IgA_MShared <- subset(IgA_M, Name%in%c(shared))
Test_IgA <- cbind(DivCal_Sort_IgA, IgA_MShared)
Test_IgA2 <- Test_IgA[,c("Observed", "InvSimpson", "Time_Corr", "IgA", "T1Dstatus")]

Test_IgA2_B2 <- subset(Test_IgA2, Time_Corr%in%c("B2"))
Test_IgA2_V1 <- subset(Test_IgA2, Time_Corr%in%c("V1"))

#B2
#Correlation Observed
cor.test(Test_IgA2_B2$Observed, Test_IgA2_B2$IgA, method = "spearman")

#Spearman's rank correlation rho

#data:  Test_IgA2_B2$Observed and Test_IgA2_B2$IgA
#S = 8615, p-value = 0.2338
...
#V1
#Correlation Observed
cor.test(Test_IgA2_V1$Observed, Test_IgA2_V1$IgA, method = "spearman")

#Spearman's rank correlation rho

#data:  Test_IgA2_V1$Observed and Test_IgA2_V1$IgA
#S = 14017, p-value = 0.1015
#alternative hypothesis: true rho is not equal to 0
#sample estimates:
#      rho 
#0.2392109 
...
#Correlation InvSimpson
#B2
cor.test(Test_IgA2_B2$InvSimpson, Test_IgA2_B2$IgA, method = "spearman")

#data:  Test_IgA2_B2$InvSimpson and Test_IgA2_B2$IgA
#S = 8772, p-value = 0.186
#alternative hypothesis: true rho is not equal to 0
#sample estimates:
#       rho 
#-0.2285714 
...
#Correlation InvSimpson
#V1
cor.test(Test_IgA2_V1$InvSimpson, Test_IgA2_V1$IgA, method = "spearman")

#data:  Test_IgA2_V1$InvSimpson and Test_IgA2_V1$IgA
#S = 16620, p-value = 0.5067
#alternative hypothesis: true rho is not equal to 0
#sample estimates:
#       rho 
#0.09791576 
...
emm_df2 <- emm_df[emm_df$Time_Corr %in% c("B1", "B2", "V1", "V2"), ]

Richnes_Abs <- ggplot(emm_df, aes(x = Time_Corr, y = response, color = t1dfactor, group = t1dfactor)) +   geom_smooth(se = FALSE, method = "loess", span = 0.8) + scale_color_manual(values = c("#4A78B5", "#C65A57")) + scale_fill_manual(values = c("#4A78B5", "#C65A57")) + theme_classic() +  labs(x = NULL, y = "Milk Microbiome Richness") + theme(axis.text.y = element_blank()) + scale_x_discrete(labels = c(B1 = "Birth",B2 = "Week 1",V1 = "Month 3", V2 = "Month 6", V3 = "Month 9", V4 = "Month 12",V5 = "Month 15"))  + theme(legend.position='none')  + theme(axis.text.x = element_text(size = 12, color = "black"), axis.title.y = element_text(size = 15, color = "black"), panel.grid.major = element_blank(),  panel.grid.minor = element_blank(),  axis.line.x = element_line(size = 0.8, color = "black"), axis.line.y = element_line(size = 0.8, color = "black"),  panel.border = element_blank(),   plot.title = element_blank(), axis.ticks = element_line() ) + geom_vline(xintercept = 2, linetype = "dashed")
```
...
#For graphical abstract
#emm_df2 <- PP2[PP2$Time_Corr %in% c("B1", "B2", "V1", "V2"), ]

SSpecies_Abs <- ggplot(PP2, aes(x = Time_Corr, y = MEAN, color = T1Dstatus, group = T1Dstatus)) +   geom_smooth(se = FALSE, method = "loess", span = 0.8) + scale_color_manual(values = c("#4A78B5", "#C65A57")) + scale_fill_manual(values = c("#4A78B5", "#C65A57")) + theme_classic() +  labs(x = NULL, y = expression(italic("Streptococcus spp. abundance")) )+ theme(axis.text.y = element_blank()) +  scale_x_discrete(labels = c(B1 = "Birth",B2 = "Week 1",V1 = "Month 3", V2 = "Month 6", V3 = "Month 9", V4 = "Month 12",V5 = "Month 15"))  + theme(legend.position='none')  + theme(axis.text.x = element_text(size = 12, color = "black"), axis.title.y = element_text(size = 15, color = "black"), panel.grid.major = element_blank(),  panel.grid.minor = element_blank(),  axis.line.x = element_line(size = 0.8, color = "black"), axis.line.y = element_line(size = 0.8, color = "black"),  panel.border = element_blank(),   plot.title = element_blank(), axis.ticks = element_line() )  + geom_vline(xintercept = 2, linetype = "dashed")

AbstractPlots <- ggarrange(Richnes_Abs, SSpecies_Abs, ncol=1, font.label = list(size = 20))
