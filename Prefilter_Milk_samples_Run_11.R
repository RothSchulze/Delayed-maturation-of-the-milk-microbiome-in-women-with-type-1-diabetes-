...
### Import data 
Mother_Milk <- import_biom("./table-dada2_QR11G.biom", parseFunction = parse_taxonomy_greengenes)

#It seemed to be working oK, without parsing the taxonomy, but I prefer to do it in the same way as the other runs.
#Mother_Milk <- import_biom("./table-dada2_QR11_wTaxMet.biom", parseFunction = parse_taxonomy_greengenes)
#tax_table(Mother_Milk) <- tax_table(Mother_Milk)[, c(1:6,8)]
#colnames(tax_table(Mother_Milk))[1] <- "Kingdom"

## Metadata
New_sample_data <- read.table(file="./Run_11_Mapping_file_Run11_Good.csv", fill=TRUE, header=TRUE, sep="\t", row.names=1)
...
New_sample_data2 <- read.table(file="./Run_11_Mapping_file_Run11_Merged_Good.csv", fill=TRUE, header=TRUE, sep="\t", row.names=1)
...
#Parse taxonomy
Tax <- data.frame(read.table(file="./Taxonomy_QR11.tsv", header=TRUE, sep="\t", row.names=1))
as.character(Tax$Taxon)
taxonomy_table <- parse_phyloseq_taxonomy(Tax)
taxonomy_table

Mother_Milk_C11 <- phyloseq(otu_table(Mother_Milk_FM),  tax_table(taxonomy_table), sample_data(Mother_Milk_FM))
...
save(Mother_Milk_C11, file="./Run_11_Milk_Features_Phyloseq_Obj_Unfiltered.RData")
